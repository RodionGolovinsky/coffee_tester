...
def catboost_grid(target_col, catboost_model, dataset_path):
    ...
    with open(
            f'/home/rodion/PycharmProjects/coffee/classification/task/quality_pred_coffee/res/report_catboost_{dataset_path.split("/")[-1].split("_")[0]}_{names_in_english[target_col]}.txt',
            'a') as f:
...
catboost_grid(target_col, CatBoostClassifier(task_type="GPU", bootstrap_type='Poisson', subsample=0.4),
              '/classification/datasets_classification/dataset_summary_data/all_materials_dataset_quality.csv')
